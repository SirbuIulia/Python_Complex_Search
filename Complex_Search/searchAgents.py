...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        return len(VisitedCorners) == 4 and all(corner in VisitedCorners for corner in self.corners)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in FoundCorners:
                    visited = list(FoundCorners) + [(nextx, nexty)]
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if len(list_Of_Food) == 0:
        ...
        for Item in list_Of_Food:
            food_Distance = mazeDistance(position, Item, startingPosition)
            heuristic = food_Distance if food_Distance > heuristic else heuristic
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        "Functie realizata de Sirbu Iulia"
        if self.food[x][y]:
            return True
        return False
        util.raiseNotDefined()
